...
function Event() {
    ...
    return (
        <div className={cx("event")}>
                    ...
                    eventList?.map((item, index) => (
                        <div key={index} className={cx('event-item')}>
                            <div className={cx("top")}>
                                <div className={cx('img-item')} style={{ backgroundImage: `url(${item.image})` }}>
                                    <div className={cx('content-item')}>
                                        ...
                                        <button className={cx('read-more')}>{item.readbtn}</button>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className={cx('imgitem')}>
                                    {
                                        item.imageSmall?.map((img, i) => (
                                            <div key={i} onClick={() => handleClickPopup(img)} className={cx('img')}>
                                                <img src={img} />
                                            </div>
                                        ))
                                    }
            ...
            <div style={ open ? {display: 'block'} : {display: 'none'}} className={cx('popup-img')}>
                <div className={cx("wrapper")}>
                            {open && (
                            <div className={cx("img-info")}>
                                <div className={cx("block-image")}>
                                <button onClick={() => setOpen(false)} className={cx("btn-close")}>X</button>
                                    <div>
